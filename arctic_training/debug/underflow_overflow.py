...
class DebugGradients:
    """
    Similar to DebugUnderflowOverflow but logs/watches params + gradient inputs and outputs

    ```python
    debug_grad = DebugGradients(model)
    ```
    ...
    def create_frame(self, module, grad_inputs, grad_outputs):
        self.expand_frame(f"{self.prefix} {self.module_names[module]} {module.__class__.__name__}")
        ...
        if isinstance(grad_inputs, tuple):
            for i, x in enumerate(grad_inputs):
                self.analyse_variable(x, f"grad_inputs[{i}]")
        else:
            self.analyse_variable(grad_inputs, "grad_inputs")

        # grad_outputs
        if isinstance(grad_outputs, tuple):
            for i, x in enumerate(grad_outputs):
                # possibly a tuple of tuples
                if isinstance(x, tuple):
                    for j, y in enumerate(x):
                        self.analyse_variable(y, f"grad_outputs[{i}][{j}]")
                else:
                    self.analyse_variable(x, f"grad_outputs[{i}]")
        else:
            self.analyse_variable(grad_outputs, "grad_outputs")
    ...
    def _register_backward_hook(self, module):
        # there is also register_post_accumulate_grad_hook but for each leaf param (not module)
        # https://docs.pytorch.org/docs/stable/generated/torch.Tensor.register_post_accumulate_grad_hook.html
        # should try this one as well, but need to check the param is a leaf param
    ...
    def backward_hook(self, module, grad_inputs, grad_outputs):
        ...
        self.create_frame(module, grad_inputs, grad_outputs)
