...
ORIGINAL_TO_CONVERTED_KEY_MAPPING = {
    r"norm.weight":                 r"norm.weight",   # noqa
    r"\nnorm.scale":                 r"\nnorm.weight",   # noqa
    r"unembedding.weight":          r"lm_head.weight",   # noqa
    r"embedding":                   r"embed_tokens",   # noqa
    # special key, wqkv needs to be split afterwards
    r"block.(\d+).attn.qkv":        r"layers.\1.self_attn.qkv_proj",   # noqa
    r"block.(\d+).attn.out":        r"layers.\1.self_attn.o_proj",   # noqa
    r"block.(\d+).attn.sinks":      r"layers.\1.self_attn.sinks",   # noqa
    r"block.(\d+).attn.norm.scale":       r"layers.\1.input_layernorm.weight",   # noqa

    r"block.(\d+).mlp.mlp1_weight": r"layers.\1.mlp.experts.gate_up_proj",   # noqa
    r"block.(\d+).mlp.mlp1_bias":   r"layers.\1.mlp.experts.gate_up_proj_bias",   # noqa
    r"block.(\d+).mlp.mlp2_weight": r"layers.\1.mlp.experts.down_proj",   # noqa
    r"block.(\d+).mlp.mlp2_bias":   r"layers.\1.mlp.experts.down_proj_bias",   # noqa
    r"block.(\d+).mlp.norm.scale":        r"layers.\1.post_attention_layernorm.weight",   # noqa
    r"block.(\d+).mlp.gate":        r"layers.\1.mlp.router",   # noqa
} # noqa
# fmt: on
...
class GptOssConverter(TikTokenConverter):
    def extract_vocab_merges_from_model(self, tiktoken_url: str):
        ...
        merges = [(token_bytes_to_string(val[0]), token_bytes_to_string(val[1])) for val in merges]  # type: ignore[misc]
        return vocab, merges
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.additional_special_tokens = {}  # type: ignore[var-annotated]
        ...
        self.additional_special_tokens = [tok for tok, _ in sorted(special_tokens_map.items(), key=lambda x: x[1])]  # type: ignore[assignment]
...
{%- endif -%}"""
    ...
    converter = GptOssConverter(
        ...
        model_max_length=None,  # type: ignore[arg-type]
