...
def assert_no_cuda_mismatch(name=""):
    ...
    if sys_cuda_version != torch_cuda_version:
        if (
            ...
            print(
                f"{WARNING} DeepSpeed Op Builder: Installed CUDA version {sys_cuda_version} does not match the version"
                f" torch was compiled with {torch.version.cuda}.Detected `DS_SKIP_CUDA_CHECK=1`: Allowing this"
                " combination of CUDA, but it may result in unexpected behavior."
            )
            return True
...
class OpBuilder(ABC):
    ...
    def load(self, verbose=False):
        ...
        from deepspeed.git_version_info import accelerator_name
        ...
        if self.jit_mode:
            ...
            if cross_compile_archs_env is not None:
                if cross_compile_archs is not None:
                    print(
                        f"{WARNING} env var TORCH_CUDA_ARCH_LIST={cross_compile_archs_env} overrides"
