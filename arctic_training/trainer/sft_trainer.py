...
class SFTTrainer(Trainer):
    ...
    def loss(self, batch) -> torch.Tensor:
        ...
        #loss_variant = "tiling"
        loss_variant = "liger_fused"

        if loss_variant == "tiling":
            ...
            def fused_logits_loss_fn(model_with_head=None, hidden_states=None, labels=None, shift_labels=None, vocab_size=0):
                logits = model_with_head.lm_head(hidden_states)

                loss = model_with_head.loss_function(logits=logits, labels=labels, vocab_size=vocab_size, shift_labels=shift_labels)
                return loss
