...
class OpenWebMath(DataSource):
    ...
    def load_fn(self, num_proc: int, eval: bool) -> Dataset:
        if eval:
            assert False, "Test split does not exist."
        dataset = load_dataset("open-web-math/open-web-math", split="train")
        subset = dataset.select(range(500000 * 3))
