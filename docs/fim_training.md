...
For FIM training with `prompt`/`response` datasets, **no special type is needed**:

```yaml
data:
  sources:
    - name_or_path: /path/to/dataset
      # No 'type' needed - format is auto-detected from columns
...
The default data source (`huggingface_instruct`) automatically detects and handles
both `prompt`/`response` and `messages` formats.

### How Auto-Detection Works

The system detects format at two levels:

1. **Data Source Level** (`HFDataSourceInstruct.post_load_callback`):
   - If dataset has `prompt` + `response` columns → passes through unchanged
   - If dataset has other columns → converts to `messages` format using `role_mapping`

2. **Tokenization Level** (`SFTDataFactory.process`):
   - If dataset has `prompt` + `response` → uses length-based label masking
   - If dataset has `messages` → uses character-offset label masking

### Data Source Types (Advanced)

For most users, no `type` is needed. These are available for explicit control:

| Type | Default | Columns | Use Case |
|------|---------|---------|----------|
| `huggingface_instruct` | **Yes** | Auto-detects both formats | FIM or SFT |
| `huggingface_prompt_response` | No | `prompt`, `response` only | Explicit FIM |
| `huggingface` | No | varies | General datasets |

**When to specify `type: huggingface_prompt_response`:**
- You want strict validation that `prompt`/`response` columns exist
- You want to ensure no `role_mapping` conversion is attempted

**When to NOT specify a type (recommended):**
- Dataset generated by `csv_to_hf_fim.py` (has `prompt`/`response`)
- Standard instruction datasets (have `messages`)
- The default handles both correctly
