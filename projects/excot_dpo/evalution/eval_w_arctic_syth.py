...
class OSSEvalVLLM:
    ...
    def batch_dataset(self, questions: Dataset, db_desc_str: Dict):
        ...
        output_data_raw = self.client.execute_batch_task(self.task_name)

        for out_i in output_data_raw:
            custom_id = int(out_i["custom_id"].split("_")[-1])
            output_data[custom_id]["model_output"] = out_i["response"][0]
        return output_data
...
def calculate_metric_bird(eval_results):
    ...
    moderate_acc = sum([1 for eval_res in moderate_eval_results if eval_res['correctness']]) / len(moderate_eval_results)
    challenging_acc = sum([1 for eval_res in challenging_eval_results if eval_res['correctness']]) / len(challenging_eval_results)
...
def main():
    ...
    if data_config.task == "bird":
        eval_results = eval_bird(args.model_name, args.prompt_version,
        data_config, task_name=args.task_name, cache_dir=data_config.cache_dir, set_type=args.mode)
        ...
        eval_results = eval_spider(args.model_name, args.prompt_version,
            data_config, task_name=args.task_name, set_type=args.mode)
